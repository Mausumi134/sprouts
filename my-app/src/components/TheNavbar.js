...
import { AuthContext } from '../context/AuthProvider'; // Adjust the import path based on your structure

const TheNavbar = () => {
  const { user, logOut } = useContext(AuthContext); // Accessing AuthContext directly
                    ...
                    <div className="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
                      {user?.email && (
                        <button onClick={logOut}>
                          <FaSignOutAlt
                            className="w-5 h-5 text-white bg-gray-900 rounded-full"
                          />
                        </button>
                      )}
                    </div>
